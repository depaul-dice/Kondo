...
# GPM files
GPMfiles =['/home/rohan/newDev/Kondo/Test/Data/GPMDATA/3B-HHR.MS.MRG.3IMERG.20140313-S223000-E225959.1350.V06B.HDF5'] #sorted(glob.glob('/home/rohan/Research/Kondo/Test/Datasets/GPMDATA/*.HDF5'))[:10000]
print(GPMfiles)
...
ncdata = h5.File(GPMfiles[0])
Y,X = np.meshgrid(ncdata['Grid/lat'][:], ncdata['Grid/lon'][:])
Y = np.flipud(Y.transpose())
X = X.transpose()

# Cell areas
cell_area_km2 = np.cos(Y * np.pi/180.) * np.pi * 12713.6 / 360. * np.pi * 12756 / 360. * 0.1**2
cell_area_km2 = cell_area_km2.reshape(1800,3600,1)
...
for GPMfile in GPMfiles:
    ...
    # Precipitation
    ncdata = h5.File(GPMfile, 'r')
    precip = ncdata['Grid/precipitationCal'][:]
...
lats = ncdata['Grid/lat'][:]
lons = ncdata['Grid/lon'][:]
